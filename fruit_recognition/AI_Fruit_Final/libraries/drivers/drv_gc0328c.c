...
/*
* YUV422閿熸枻鎷烽敓鏂ゆ嫹閿熸枻鎷烽敓锟�,UYVY,杞敓鏂ゆ嫹涓篟GB565,
* inBuf -- YUV data
* outBuf -- RGB565 data
* imgWidth,imgHeight -- image width and height
* cvtMethod -- 閿熸枻鎷锋晥閿熸枻鎷烽敓鏂ゆ嫹
*/
int convert_uyvy_to_rgb(unsigned char *inBuf, unsigned char *outBuf, int imgWidth, int imgHeight)
{
    int rows ,cols; /* 閿熸枻鎷烽敓鍙唻鎷峰織 */
    int y, u, v, r, g, b;   /* yuv rgb 閿熸枻鎷疯儰閿熸枻鎷烽敓锟� */
    unsigned char *YUVdata, *RGBdata;   /* YUV閿熸枻鎷稲GB閿熸枻鎷烽敓鏂ゆ嫹鎸囬敓鏂ゆ嫹 */
    int Ypos, Upos, Vpos;   /* Y U V閿熸枻鎷烽敓鏂ゆ嫹閿熸嵎浼欐嫹閿熸枻鎷烽敓鍙鎷峰亸閿熸枻鎷� */
    unsigned int i = 0;
...
//    Vpos = Upos + 2;
    ...
    for(rows = 0; rows < imgHeight; rows++)
    {
        for(cols = 0; cols < imgWidth; cols++)
        {
            ...
            /* 閿熸帴浣庣鎷烽敓鏂ゆ嫹r g b */
            *(RGBdata ++) = (((g & 0x1c) << 3) | (b >> 3)); /* g閿熸枻鎷�5浣嶉敓鏂ゆ嫹b閿熸枻鎷�5浣� */
            *(RGBdata ++) = ((r & 0xf8) | (g >> 5));    /* r閿熸枻鎷�5浣嶉敓鏂ゆ嫹g閿熸枻鎷�3浣� */

            /* 閿熸枻鎷烽敓鏂ゆ嫹閿熻鏂ゆ嫹閿熸枻鎷烽敓鏂ゆ嫹閿熷彨甯嫹閿熸枻鎷蜂竴閿熸枻鎷穀 */
            Ypos += 2;
            //Ypos++;
            i++;
            /* 姣忛敓鏂ゆ嫹閿熸枻鎷穀閿熸枻鎷烽敓鏂ゆ嫹涓�閿熸枻鎷稶V */
            if(!(i & 0x01))
            {
                Upos = Ypos - 1;
                Vpos = Ypos + 1;
...
static void rgb565_to_rgb888(rt_uint16_t *rgb565Color, rt_uint8_t *rgb888Color, int size)
{
    ...
    {
        color = rgb565Color[i];
        // 閿熸枻鎷峰彇RGB閿熸枻鎷疯壊閿熸枻鎷烽敓鏂ゆ嫹閿熸枻鎷烽敓鏂ゆ嫹浣�
...
void gc0328c_rgb565_mode(void)
{
    ...
    write_reg(i2c_bus, 0x49, 0x23);  //閿熷彨浼欐嫹閿熸枻鎷峰皬閿熸枻鎷�
    write_reg(i2c_bus, 0x44, 0x26);  //閿熷彨浼欐嫹閿熸枻鎷稲GB565閿熸枻鎷烽敓锟�
}
...
//    rt_thread_delay(20);
    ...
    result = camera_inf_cfg_gc0328c_fps(TYPE_5FPS);
    if(result != RT_EOK)
    {
